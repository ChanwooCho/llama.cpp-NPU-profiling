...
static thread_ret_t ggml_graph_compute_thread(void * data) {
    ...
    double ffn_end_time = 0;
    double compute_start_time = 0 ;
    double compute_end_time = 0;
    double compute_duration = 0;
    double synch_start_time = 0;
    double synch_end_time = 0;
    double synch_duration = 0;
    ...
    for (int node_n = 0; node_n < cgraph->n_nodes && atomic_load_explicit(&tp->abort, memory_order_relaxed) != node_n; node_n++) {
        ...
        {   
            ...
            //     printf("%dth thread, %d core *classifier*  = %f ms\n", state->ith, cpu, compute_duration + synch_duration);
            // }
        }
    }
